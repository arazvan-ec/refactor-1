...
test_infection:
	...
	@if [ -z $${$(VAR_NAME_IS_DOCKER)+x} ]; then \
    	$(DOCKER_EXEC) php -d disable_functions=  ./bin/infection --min-msi=50 --min-covered-msi=90 --threads=4; \
    else \
			php -d disable_functions= ./bin/infection --min-msi=50 --min-covered-msi=90 --threads=4; \
  fi
